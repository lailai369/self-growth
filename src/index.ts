  ...
  if (!_initPromise) {
    _initPromise = (async () => {
      ...
      try {
        ...
        if ((activation as any)?.email) {
          const res = await fetch(`${CONFIG.CLOUD_URL}/api/auth/verify?email=${encodeURIComponent((activation as any).email)}`, { signal: AbortSignal.timeout(5000) });
          if (res.ok) {
            const data = await res.json() as any;
...
3. 话题突然切换，上一个任务没完成
4. 用户直接离开或沉默结束
符合任一条件就提取。每行一条。无则输出"无"。`,
    600
  );
  const result = {
    preferences: [] as any[], lessons: [] as any[], longTermGoals: [] as any[],
    experiences: [] as any[], interruptedTasks: [] as any[]
  };
  if (!answer) return result;
  for (const line of answer.split('\n')) {
    if (line.startsWith('[长期]')) result.longTermGoals.push({ text: line.slice(4).trim(), type: 'decision', source: 'llm_reflection' });
    ...
    else if (line.startsWith('[中断任务]')) result.interruptedTasks.push({ text: line.slice(6).trim() });
  }
  return result;
}

async function saveInterruptedTasks(basePath: string, taskText: string): Promise<void> {
  ...
  if (existing.includes(taskText.substring(0, 30))) return;
  await fs.writeFile(filePath, existing + entry, 'utf-8');
}

function readInterruptedTasks(basePath: string): string {
  try {
...
async function removeInterruptedTask(basePath: string, taskText: string): Promise<void> {
  const filePath = path.join(basePath, 'memory', 'interrupted_tasks.md');
  try {
  ...
  register(api: any) {
    ...
    api.registerTool({
      ...
      async execute(_runId: any, params: any) {
        await ensureInit();
        if (!_state!.isPro) {
          return { details: {}, content: [{ type: "text" as const, text: "🔒 每日复盘是 Pro 功能，请升级套餐。访问 http://115.28.208.50/setup.html" }] };
        }
    ...
    api.registerTool({
      ...
      async execute(_runId: any, params: any) {
        await ensureInit();
        const p = params as { action: string; task?: string };
        const basePath = getBasePath();
        if (p.action === "add" && p.task) { await saveInterruptedTasks(basePath, p.task); return { details: { recorded: true }, content: [{ type: "text" as const, text: "✅ 任务已暂存。" }] }; }
        if (p.action === "complete" && p.task) { await removeInterruptedTask(basePath, p.task); return { details: { recorded: true }, content: [{ type: "text" as const, text: "✅ 任务已完成。" }] }; }
        if (p.action === "list") { const tasks = readInterruptedTasks(basePath); return { details: { tasks }, content: [{ type: "text" as const, text: tasks || "暂无中断任务。" }] }; }
        return { details: {}, content: [{ type: "text" as const, text: "❌ 无效操作。" }] };
      }
    });
...
`);
      ...
      try {
        ...
        if (userText && now - _lastReflectionTime > 10000) {
          ...
          unifiedReflection(userText, agentText).then(async r => {
            ...
            if (r.longTermGoals.length > 0) await s.memoryManager.addPreferences(r.longTermGoals);
            if (r.interruptedTasks.length > 0) {
              for (const t of r.interruptedTasks) await saveInterruptedTasks(s.basePath, t.text);
            }
          }).catch(() => {});
        }
    ...
    api.on("before_compaction", (event: any) => {
      ...
      const recentAgentMsgs = messages.filter((m: any) => m.role === 'assistant' || m.role === 'agent').slice(-10);
      ...
      for (const m of recentAgentMsgs) {
        ...
        if (steps) progressLines.push(...steps);
      }

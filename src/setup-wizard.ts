...
import { loadActivation, saveActivation, PlanType } from './payment';

export interface WizardStep {
  title: string;
  description: string;
  options: { key: string; label: string; plan?: PlanType }[];
}
...
export async function shouldShowWizard(basePath: string): Promise<boolean> {
  try {
...
export async function markWizardComplete(basePath: string): Promise<void> {
  await fs.writeFile(path.join(basePath, WELCOME_FILE), new Date().toISOString(), 'utf-8');
}

export async function downloadCloudClient(basePath: string): Promise<string | null> {
...
export function getWelcomeStep(): WizardStep {
  return {
    title: '欢迎使用 yulailai',
...
export function getPlanStep(): WizardStep {
  return {
    title: '选择套餐',
...
export function getPaymentStep(plan: PlanType, amount: number): WizardStep {
  const prices: Record<PlanType, string> = { free: '免费', pro: '¥15/月', enterprise: '¥3000/年' };
  return {
    title: '扫码支付',
...
export function getCompleteStep(plan: PlanType): WizardStep {
  const names: Record<PlanType, string> = { free: '免费版', pro: '专业版', enterprise: '企业版' };
  return {
    title: '设置完成',
